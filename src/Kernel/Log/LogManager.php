...
class LogManager implements LoggerInterface
{
    ...
    {
        return new Monolog('Enjelly', $this->prepareHandlers([new StreamHandler(
            \sys_get_temp_dir().'/Enjelly/Enjelly.log', $this->level(['level' => 'debug'])
        )]));
    }
    ...
    protected function createSyslogDriver(array $config)
    {
        return new Monolog($this->parseChannel($config), [
            $this->prepareHandler(new SyslogHandler(
                    'Enjelly', $config['facility'] ?? LOG_USER, $this->level($config))
            ),
        ]);
    }
