...
use Enjelly\Kernel\Support\AES;
use Throwable;
// use function Enjelly\Kernel\Support\str_random;
...
class Encryptor
{
     ...
     */
    public function __construct(string $corpId, string $token = null, string $aesKey = null)
    {
        $this->corpId = $corpId;
        $this->token = $token;
     ...
     * @return string
     *
     * @throws \Enjelly\Kernel\Exceptions\RuntimeException
     */
    public function encrypt($plain, $nonce = null, $timestamp = null): array
    {
        try {
            $plain = $this->pkcs7Pad(str_random(16).pack('N', strlen($plain)).$plain.$this->corpId, $this->blockSize);
        ...
        return $response = [
            'encrypt' => $encrypted,
            'msg_signature' => $this->signature($this->token, $timestamp, $nonce, $encrypted),
            'timeStamp' => $timestamp,
            'nonce' => $nonce,
        ];
    }
    ...
    public function decrypt($content, $msgSignature, $nonce, $timestamp): string
    {
        ...
        if (trim(substr($content, $contentLen + 4)) !== $this->corpId) {
