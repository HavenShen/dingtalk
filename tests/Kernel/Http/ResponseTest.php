...
namespace Enjelly\Tests\Kernel\Http;
...
class ResponseTest extends TestCase
{
    ...
    {
        $response = new Response(200, ['content-type:application/json'], '{"name": "Enjelly"}');

        $this->assertInstanceOf(\GuzzleHttp\Psr7\Response::class, $response);

        $this->assertSame('{"name": "Enjelly"}', (string) $response);
        $this->assertSame('{"name": "Enjelly"}', $response->getBodyContents());
        $this->assertSame('{"name":"Enjelly"}', $response->toJson());
        $this->assertSame(['name' => 'Enjelly'], $response->toArray());
        $this->assertSame('Enjelly', $response->toObject()->name);
        $this->assertInstanceOf(Collection::class, $response->toCollection());
        $this->assertSame(['name' => 'Enjelly'], $response->toCollection()->all());
    }
